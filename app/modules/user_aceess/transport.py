...
def get_all_access_transport(session: Session, user: User):
    """Получает все uNumber транспорта, доступные пользователю."""
    if user.role == 1:
        return [t.uNumber for t in session.query(Transport).all()]
    ...
    query =  session.query(Transport).outerjoin(Storage, Transport.storage_id == Storage.ID)
