...
class Transport(Base):
    ...
    storage = relationship('Storage', back_populates='transports', primaryjoin="Storage.ID == Transport.storage_id")
    uNumber = Column(Text)
...
class Alert(Base):
    ...
    type = Column(Text(50), ForeignKey('alert_type.alert_un'), nullable=False)
    ...
    alert_type = relationship('AlertType', backref='alerts')
...
class FunctionalityAccess(Base):
...
def insert_mailing_record_sqlalchemy(target, subject, content, html_template=None, attachment_name=None, attachment_content=None, session: Session=None):
    try:
        sql = text("""
...
def reports_custom_transport_transfer(session: Session, start_date: str, end_date: str, region: str, home_storage):
    try:
        sql = text("""
